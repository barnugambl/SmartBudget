...
class LogView: UIView {
    ...
    private lazy var logButton = DefaultButton(title: R.string.localizable.logButton(), buttonBackgroundColor: .white, layerBorderWidth: 1,
                                               layerBorderColor: UIColor.systemGray.cgColor) { [weak self] in
        self?.onEvent?(.didTapLogin)
    }
    
    private lazy var regButton = DefaultButton(title: R.string.localizable.regButton(), buttonBackgroundColor: .systemGray5) { [weak self] in
    ...
    private lazy var formFieldStackView = Stack(stackSpaicing: 32, views: [phoneNumberTextField, passwordTextField])
    ...
    private func setupLayout() {
        ...
        forgotButtonPassword.snp.makeConstraints { make in
            make.top.equalTo(formFieldStackView.snp.bottom).offset(8)
